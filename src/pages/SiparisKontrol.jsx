...
function ScanSession({ items, irsaliyeInfo, onDone, onBack }) {
  ...
  const totalScanned = items.reduce((a,i)=>a+(counts[i.ean || i.malzemeKodu]||0),0);
          ...
          <div style={{ position:'relative',background:'#000',height:160 }}>
            ...
            {!camOn && (
              <div style={{ position:'absolute',inset:0,background:'rgba(0,0,0,.7)',display:'flex',alignItems:'center',justifyContent:'center' }}>
                <p style={{ color:'rgba(255,255,255,.5)',fontSize:13 }}>📷 Kamera başlatılıyor...</p>
              </div>
            )}
          ...
          <div style={{ padding:'10px 10px 6px' }}>
            <div style={{ display:'flex',gap:7 }}>
              ...
              <button
                ...
                style={{ background:'linear-gradient(135deg,#3b82f6,#6366f1)',color:'#fff',border:'none',padding:'0 16px',borderRadius:10,fontWeight:700,cursor:'pointer' }}
              >
                Tara
              </button>
            </div>
        ...
        {visible.map(item=>{
          ...
          return (
            <div key={key} style={{ background:st.card,border:`1px solid ${st.border}`,borderRadius:14,padding:'11px 12px',marginBottom:7 }}>
              ...
              {item.beklenen > 1 && (
                <div style={{ marginTop:7,marginLeft:17,height:3,background:'#e2e8f0',borderRadius:4,overflow:'hidden' }}>
      ...
      <div style={{ padding:'16px',maxWidth:500,margin:'0 auto' }}>
        {loading ? (
          <div style={{ textAlign:'center',padding:'60px 0' }}>
            ...
            <p style={{ fontSize:13,color:'#94a3b8',marginTop:4 }}>İrsaliye analiz ediliyor...</p>
          </div>
        ) : (
          <>
            <div
            ...
            <div style={{ background:'#f8fafc',borderRadius:14,padding:'14px 16px',border:'1px solid #e2e8f0' }}>
              ...
              {[
                ...
                <p key={i} style={{ color:'#475569',fontSize:12,marginBottom:5,display:'flex',gap:8 }}>
